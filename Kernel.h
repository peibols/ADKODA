...
class Pgg : public Kernel { //g --> g + g
  public:
    using Kernel::Kernel;
    double Value (double z, double y) {return CA / 2. * ( 2./(1.-z) - 2. + z*(1.-z) );} //FIXME extra 1/2, not symmetrized in z?
...
class Pgq : public Kernel { //g --> q + qbar
  public:
    using Kernel::Kernel;
    double Value ( double z, double y ) {return TR * 0.5 * (z*z + (1.-z)*(1.-z)); } //FIXME there is a 1/2 difference in AP and CS.
    double Estimate ( double z ) {return TR * 0.5;}
    double Integral ( double zm, double zp ) {return TR * 0.5 * (zp - zm);}
